...
export function Sidebar() {
  ...
  return (
    <aside className="sidebar" aria-label="SDK navigation">
          ...
          return (
            <section key={g.id} className="sidebar__group">
              <div className="sidebar__group-heading">
                <span className="sidebar__group-icon" aria-hidden>
                  <GroupIcon group={g.id} size={11} />
                </span>
                {g.label}
              </div>
                  ...
                  return (
                    <li key={item.href}>
                      <Link href={item.href} className={`sidebar__link${active ? ' is-active' : ''}`}>
                        ...
                        {item.badge ? (
                          <span
                            className={`sidebar__badge sidebar__badge--${item.badge
                              .toLowerCase()
                              .replace('-', '')}`}
                          >
                            {item.badge}
